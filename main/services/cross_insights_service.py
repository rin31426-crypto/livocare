...
class HealthInsightsEngine:
    ...
    def _get_status_text(self, status_type, value=None):
        """الحصول على نص الحالة حسب النوع"""
        statuses = {
            'weight_low': {
                'ar': '⚠️ وزنك منخفض جداً',
                'en': '⚠️ Your weight is very low'
            },
            'weight_high': {
                'ar': '⚠️ وزنك مرتفع',
                'en': '⚠️ Your weight is high'
            },
            'weight_ideal': {
                'ar': '✅ وزنك في المعدل المثالي',
                'en': '✅ Your weight is ideal'
            },
            'glucose_high': {
                'ar': '🩸 سكر الدم مرتفع',
            ...
            'glucose_normal': {
                'ar': '✅ سكر الدم طبيعي',
                'en': '✅ Blood sugar is normal'
            },
            'bp_ideal': {
                'ar': '✅ ضغط الدم مثالي',
                'en': '✅ Blood pressure is ideal'
            },
            'bp_normal': {
                'ar': '✅ ضغط الدم طبيعي',
                'en': '✅ Blood pressure is normal'
            },
            'bp_high': {
                'ar': '⚠️ ضغط الدم مرتفع',
                'en': '⚠️ Blood pressure is high'
            },
            'bp_low': {
                'ar': '⚠️ ضغط الدم منخفض',
                'en': '⚠️ Blood pressure is low'
            }
        }
        return statuses.get(status_type, {}).get(self._get_lang_key(), status_type)
    
    def _get_lang_key(self):
        return 'ar' if self.is_arabic else 'en'
    ...
    def analyze_vital_signs(self):
        ...
        if glucose:
            ...
            if glucose > 140:
                ...
                if last_meal and last_meal.ingredients:
                    high_carbs = any(
                        ing.get('carbs', 0) > 30 
                ...
                insights.append({
                    'type': 'glucose',
                    'severity': 'good',
                    'message': self._t('✅ سكر الدم طبيعي', '✅ Blood sugar is normal'),
                    'details': self._t(f'{glucose} mg/dL', f'{glucose} mg/dL'),
                    'recommendation': self._t(
        ...
        if weight < 55 and weekly_activity > 200:
            ...
            if deficit > 300:
                ...
                recommendations.append({
                    ...
                    'tips': [
                        self._t('أضف وجبة خفيفة بين الوجبات الرئيسية', 'Add a snack between main meals'),
                        self._t('تناول المكسرات والفواكه المجففة', 'Eat nuts and dried fruits'),
                        self._t('اشرب الحليب كامل الدسم', 'Drink full-fat milk'),
                        self._t('أضف زيت زيتون إلى السلطة', 'Add olive oil to salad')
                    ]
                })
    ...
    def analyze_pulse_pressure(self):
        ...
        if pulse_pressure < 15:
            ...
            if last_sleep and last_sleep.duration_hours and float(last_sleep.duration_hours) < 6:
                alert['causes'].append(
        ...
        if glucose and glucose < 100 and has_activity_today:
            ...
            if glucose < 80:
                recommendations.append({
                    ...
                    'details': self._t(
                        f'نسبة السكر {glucose} mg/dL قد تنخفض أكثر أثناء التمرين',
                    ...
                    'food_suggestions': [
                        self._t('موزة', 'Banana'),
                        self._t('تمر (3-4 حبات)', 'Dates (3-4 pieces)'),
                        self._t('عصير فواكه', 'Fruit juice'),
                        self._t('شريحة توست مع عسل', 'Toast with honey')
                    ]
                })
            elif glucose < 100:
                recommendations.append({
                    ...
                    'advice': self._t(
                        'تناول وجبة خفيفة قبل التمرين لتجنب الدوخة',
        ...
        if systolic and diastolic:
            if systolic > 140 or diastolic > 90:
                ...
                recommendations.append({
                    ...
                    'tips': [
                        self._t('اشرب 500 مل ماء', 'Drink 500ml water'),
                        self._t('تناول وجبة خفيفة مالحة', 'Eat a light salty snack')
                    ]
                })
    ...
    def analyze_activity_nutrition(self):
        ...
        return {
            ...
            'analysis': analysis,
            'summary': self._summarize_activity_nutrition(analysis)
        }
    ...
    def analyze_sleep_mood(self):
        ...
        for sleep in sleep_records:
            ...
            if next_day_mood and sleep.duration_hours:
        ...
        return {
            ...
            'recommendation': self._generate_sleep_recommendation(mood_sleep_avg)
        }
    ...
    def analyze_weight_trends(self):
        ...
        if days_diff > 0:
            ...
            return {
                ...
                'prediction': self._predict_weight(weight_change, daily_rate, days_diff)
            }
    ...
    def analyze_blood_pressure(self):
        ...
        category = self._classify_blood_pressure(avg_sys, avg_dia)
        
        return {
            'average': f"{avg_sys:.0f}/{avg_dia:.0f} mmHg" if avg_sys and avg_dia else None,
            'category': category,
            'patterns': changes,
            'recommendation': self._get_bp_recommendation(avg_sys, avg_dia, changes)
        }
    ...
    def analyze_glucose_risks(self):
        ...
        risk_score = self._calculate_diabetes_risk(avg_glucose, max_glucose, trend)
    ...
    def _summarize_activity_nutrition(self, analysis):
    ...
    def _generate_sleep_recommendation(self, mood_sleep_avg):
    ...
    def _classify_blood_pressure(self, sys, dia):
        """تصنيف ضغط الدم"""
    ...
    def _get_bp_recommendation(self, sys, dia, patterns):
        """توصية لضغط الدم"""
        if not sys or not dia:
        ...
        category = self._classify_blood_pressure(sys, dia)
    ...
    def _calculate_diabetes_risk(self, avg, max_val, trend):
        """حساب مخاطر السكري"""
        risk = 0
    ...
    def _predict_weight(self, change, daily_rate, days):
